...
CHECKPOINT_PATH = sys.argv[1] if len(sys.argv) > 1 else "checkpoints/generalist/gru_R+M+LvR+M+L_beta0.5_ep150.pth"
...
test_dict = {t: np.load(f"datasets/{t}_dict.npy", allow_pickle=True).item() for t in test_tasks}
out_by_task = {}
for task in test_tasks:
    data = np.array([test_dict[task][pid] for pid in test_ids])  # [patients, frames, 6]
    data = (data - mu) / sigma
    ds = TimeSeriesDataset(data, test_ids, device=device)
    loader = GPUBatchLoader(ds, batch_size=1024, shuffle=False)
    out_by_task[task] = evaluate(model, loader, mu, sigma, device, sigma_threshold=sigma_threshold)
...
save(plots.error_per_dimension(pred, true, base, task_labels, test_tasks), "01_error_per_dimension")
save(plots.true_vs_predicted(pred, true, task_labels, test_tasks), "02_true_vs_predicted")
save(plots.fd_distribution(fd_pred, fd_base, task_labels, test_tasks), "03_fd_distribution")
save(plots.per_patient_fdg(fd_per_patient_pred, fd_per_patient_base, fdg_per_patient, test_tasks), "04_per_patient_fdg")
save(plots.metrics_summary(pred, true, base, task_scalars, test_tasks, tag), "05_metrics_summary")
...
tp, pp, bp, sp = out["true"][sel].copy(), out["pred"][sel].copy(), out["base"][sel].copy(), out["std"][sel].copy()
for a in (tp, pp, bp, sp):
    a[:, 3:6] *= 50
save(plots.patient_timeseries(tp, pp, bp, sp, patient_id, viz_task), "07_patient_timeseries")

save(plots.fdgain_vs_motion(fd_pred, fd_base, task_labels, test_tasks), "08_fdgain_vs_motion")
