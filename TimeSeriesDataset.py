...
class TimeSeriesDataset(Dataset):
    ...
    def __init__(self, data, sequence_length=10, TRANSFORM=False):
        """
        Args:
    ...
    def __getitem__(self, index):
        n = index // (self.T - self.time_span + 1)  # Patient index
        t = index % (self.T - self.time_span + 1)  # Time index

        x = self.data[n, t : t + self.time_span : 2, :]  # Sub-sequence
        y = self.data[n, t + (self.time_span) - 1, :]  # Next time step
