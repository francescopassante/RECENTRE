...
Usage: python robustness.py [checkpoints/beta_scan]
"""
...
CHECKPOINT_PATH = sys.argv[1] if len(sys.argv) > 1 else "checkpoints/beta_scan"
...
def run_eval(checkpoint_path, noise_level):
    """Evaluate one checkpoint over all its test tasks with noise; return per-run metrics."""
    ...
    nll_sum = 0.0
    for task in tqdm(test_tasks, desc=f"Eval {os.path.basename(checkpoint_path)[:15]} (noise={noise_level})", leave=False):
        ...
        out = evaluate(model, loader, mu, sigma, device, noise=noise_level)
...
# evaluate each checkpoint at each noise level
per_run = defaultdict(list)
if os.path.isfile(CHECKPOINT_PATH):
    checkpoints = [CHECKPOINT_PATH]
else:
    checkpoints = sorted(glob.glob(os.path.join(CHECKPOINT_PATH, "*.pth")))
...
fig.suptitle(f"Robustness: Headline metrics vs {GROUP_BY}")
...
fig.tight_layout()
save(fig, "01_robustness_headline")

# 2) Calibration vs noise
...
fig.tight_layout()
save(fig, "02_robustness_calibration")

# 3) Per-dimension MAE & predicted σ vs noise
...
for ax, key, ylabel, title in zip(
    ...
    arr = np.stack([results[g][key] for g in groups], axis=0)
...
fig.tight_layout()
save(fig, "03_robustness_per_dim")

# 4) FD distribution per noise level
...
ax.legend(fontsize=9)
fig.tight_layout()
save(fig, "04_robustness_fd_dist")

# 5) Summary table
fig, ax = plt.subplots(figsize=(14, 0.7 + 0.45 * len(groups)))
ax.axis("off")
header = [GROUP_BY, "NLL", "FD base", "FD model", "FDg", "MAE", "mean σ", "±1σ %", "±2σ %"]
rows = []
for g in groups:
    r = results[g]
    rows.append(
...
best = {
    1: int(np.argmin([results[g]["nll"] for g in groups])),
    3: int(np.argmin([results[g]["fd_pred"] for g in groups])),
    4: int(np.argmax([results[g]["fdg"] for g in groups])),
    5: int(np.argmin([results[g]["mae_total"] for g in groups])),
    7: int(np.argmin([abs(results[g]["cov_1sigma"] * 100 - 68.27) for g in groups])),
    8: int(np.argmin([abs(results[g]["cov_2sigma"] * 100 - 95.45) for g in groups])),
}
for col, row in best.items():
    tbl[(row + 1, col)].set_facecolor("lightgreen")

fig.suptitle(f"Robustness sweep summary (green = best per column)")
fig.tight_layout()
save(fig, "05_robustness_summary")
